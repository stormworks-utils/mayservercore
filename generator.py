...
def generate(path: Path, extract=True, http_port=1000, update=False, write_full_traceback=False):
    ...
    '''
        ...
        if 'httpGet' in functions.keys():
            http = handlers.generate_handler('httpget', {'PORT':http_port}, [{'PREFIX':x.split('_')[1], 'NAME':moduleprefixes[x.split('_')[1]], 'PREFIXED':x} for x in functions['httpGet']])
            script += http
        if 'httpReply' in to_handle.keys():
            http = handlers.generate_handler('httpreply', {'PORT':http_port}, [{'PREFIX':x.split('_')[1], 'NAME':moduleprefixes[x.split('_')[1]], 'PREFIXED':x} for x in to_handle['httpReply']])
            script += http
        ...
        svr=handlers.generate_handler('flask_handler',{'MODULE_LIST':str(extended_modules), 'PORT':http_port},[],repl_head=True)
