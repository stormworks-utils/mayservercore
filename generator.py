...
def generate(path: Path, extract=True, http_port=1000, update=False, write_full_traceback=False):
    ...
    try:
        ...
        for module in modules:
            ...
            if name == '':
                continue
            compiled_modules += f'''--{modulealias[name]}: {desc}
    ...
    '''
        ...
        if 'httpGet' in functions.keys():
            http = handlers.generate_handler('httpget', {'PORT':http_port}, [{'PREFIX':x.split('_')[1], 'NAME':modulealias[name], 'PREFIXED':x} for x in functions['httpGet']])
            script += http
        if 'httpReply' in to_handle.keys():
            http = handlers.generate_handler('httpreply', {'PORT':http_port}, [{'PREFIX':x.split('_')[1], 'NAME':modulealias[name], 'PREFIXED':x} for x in to_handle['httpReply']])
            script += http
        ...
        for module in modules:
            ...
            if has_ext:
                ...
                if not ext_has_handler:
                    log.warn(f'Python extension for module "{file_name}" does not contain a valid handler function.')
                if len(functions)>0:
                ...
                os.mkdir(f'{server_path}/py/{file_name}')
                for file in files:
                    log.info(f"Copying {file}")
                    #do public/hidden/private prefixes
                    shutil.copyfile(f'modules/{file_name}/{file}', f'{server_path}/py/{file_name}/{file}')
        log.info('Building HTTP server')
        print(server_path)
        svr=handlers.generate_handler('flask_handler',{'MODULE_LIST':str(module_names), 'PORT':http_port},[],repl_head=True)
